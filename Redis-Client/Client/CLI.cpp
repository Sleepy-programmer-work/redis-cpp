...
void CLI::run(const std::vector<std::string>& commandArgs) {
    ...
    // Install readline handler once before the loop
    ...
    while (true) {
        ...
        if (lineReady) {
            ...
            std::transform(firstCmd.begin(), firstCmd.end(), firstCmd.begin(), 
            ...
            try {
                ...
                std::cerr << "\n(Error) Failed to parse response: " << e.what() << "\n";
                ...
                std::cerr << "\n(Error) Unknown error during response parsing.\n";
                std::cout.flush();
                if (readlineActive) {
                    rl_callback_handler_remove();
                    readlineActive = false;
